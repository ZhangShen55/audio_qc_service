    ...
    async def run(self, src_path: Path, file_size_bytes: int) -> QCResult:
        ...
        with tempfile.TemporaryDirectory(prefix="aqc_") as td:
            ...
            try:
                ...
                logger.debug(f"[DECODE] FFmpeg decode completed successfully. wav_path={wav_path}")
            ...
            # 读取 wav
            logger.debug(f"[AUDIO_IO] Reading WAV file. wav_path={wav_path}")
            try:
                audio = read_wav_mono_float32(wav_path)
                logger.debug(f"[AUDIO_IO] WAV file read successfully. sample_rate={audio.sample_rate}, duration_ms={audio.duration_ms}")
            ...
            # 计算静音
            logger.debug(f"[METRICS] Computing silence detection. silence_dbfs={aqc.silence_dbfs}")
            ...
            logger.debug(f"[METRICS] Speech analysis completed. speech_ms={speech_ms}, speech_ratio={speech_ratio}")

            # has_speech：可按业务再调阈值，这里给个稳妥的最小语音 300ms
            has_speech = speech_ms >= 300
            logger.debug(f"[METRICS] Speech detection result. has_speech={has_speech}")

            # 爆音次数
            logger.debug(f"[METRICS] Computing clipping events")
            clip_count = count_clipping_events(audio.wav)
            logger.debug(f"[METRICS] Clipping detection completed. clip_count={clip_count}")

            # 清晰度（V1规则版）
            if aqc.need_clarity:
                logger.debug(f"[METRICS] Computing clarity score (V1)")
                ...
                logger.debug(f"[METRICS] Clarity computation completed. clarity={clarity}, snr_db={detail.snr_db}, hf_ratio={detail.hf_ratio}, spectral_flatness={detail.spectral_flatness}")
                clarity_detail = {
                ...
                logger.debug(f"[METRICS] Clarity computation skipped (need_clarity=False)")
                clarity = None
                clarity_detail = None
